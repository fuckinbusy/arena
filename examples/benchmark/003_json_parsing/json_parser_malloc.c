...
JsonFile *json_read_file(const char *filename)
{
    ...
    JsonFile *json = (JsonFile*)malloc(sizeof(JsonFile)+file_size+1);
    if (!json) return NULL;
...
JsonValue *json_parse_array(char **p)
{
    ...
    root->array.values = malloc(sizeof(JsonValue*) * root->array.capacity);
    if (!root) goto exit_;
    ...
    while (1) { // infinite loop (its okay because it will be interrupted anyway inside)
        ...
        if (root->array.size >= root->array.capacity) {
            ...
            void *tmp = realloc(root->array.values, new_cap * sizeof(JsonValue*));
            if (!tmp) goto exit_;
...
JsonValue *json_parse_object(char **p)
{
    ...
    root->object.keys     = malloc(sizeof(char*) * root->object.capacity);
    root->object.values   = malloc(sizeof(JsonValue*) * root->object.capacity);
    ...
    while (1) {
        ...
        if (root->object.capacity <= root->object.size) {
            ...
            void *tmp_keys      = realloc(root->object.keys, root->object.capacity * sizeof(char*));
            void *tmp_values    = realloc(root->object.values, root->object.capacity * sizeof(JsonValue*));
